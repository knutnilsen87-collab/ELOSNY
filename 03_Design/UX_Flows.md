# UX Flows

## Source of truth

This document is intentionally a pointer.

The canonical Evida design source of truth is:

`03_Design/UI_UX_Product_Spec.md`

The visual MVP flow artifact is:

`03_Design/03_Evida_MVP_Figma_Flow.svg`

## Use this file for

vertical MVP flow, core user workflows, prototype connectors and flow acceptance criteria.

## Relevant canonical sections

- `03_Design/UI_UX_Product_Spec.md` section 6, 7, 8 and 15
- `03_Design/03_Evida_MVP_Figma_Flow.svg` for the clickable/prototype flow reference

## Rule

Do not update this file independently with competing product/design requirements.

If design behavior changes:
1. update `03_Design/UI_UX_Product_Spec.md`;
2. update `03_Design/03_Evida_MVP_Figma_Flow.svg` if the visual flow changed;
3. update this pointer only if the canonical section mapping changes.

## Repo-health note

This file remains in place to preserve template navigation and expected document names while avoiding duplicated or conflicting design authority.
