# Evida Documentation

All project documentation lives in this folder.

## Start here

- `00_START_HERE/README_Start_Here.md`
- `00_START_HERE/Project_Profile_Card.md`
- `MASTER_TEMPLATE_README.md`

## Product and delivery

- `02_Product/PRD_Product_Requirements_Document.md`
- `02_Product/Functional_Specification.md`
- `05_Delivery/Roadmap_and_Milestones.md`

## Design

- `03_Design/UI_UX_Product_Spec.md`
- `03_Design/03_Evida_MVP_Figma_Flow.svg`

## Technical

- `04_Technical/Technical_Architecture_Spec.md`
- `04_Technical/Canonical_Domain_Model_v1.md`
- `04_Technical/API_Specification.md`
- `04_Technical/OpenAPI_MVP_Spec.yaml`
- `04_Technical/SQL_Schema_v1.sql`

## QA, security and operations

- `06_QA/Acceptance_Criteria.md`
- `07_Operations/Local_Setup_Guide.md`
- `08_Security_Legal/Security_Requirements.md`
- `status/status_bundle.json`
